...
#if defined(NDEBUG)
# error "PocketTest cannot be compiled without assertions."
#endif
...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // PocketTest: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "PocketTest Signed Message:\n";
...
{
    // PocketTest: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 10 blocks, which will occur approximately every 4 years
    // nSubsidy >>= (nHeight / 10); // PocketTest: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 0.25 * 24 * 60 * 60; // PocketTest: 3.5 days
static const int64 nTargetSpacing = 0.5 * 60; // PocketTest: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // PocketTest: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // PocketTest: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // PocketTest: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "The time is now";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // PocketTest: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PocketTestMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PocketTestMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PocketTestMiner(CWallet *pwallet)
{
    printf("PocketTestMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("PocketTest-miner");
    ...
    try { loop {
        ...
        printf("Running PocketTestMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PocketTestMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PocketTestMiner, pwallet));
}
